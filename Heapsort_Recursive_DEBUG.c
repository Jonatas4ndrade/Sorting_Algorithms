...
// It will check if given element is in correct position, regarding a min-heap. Works in-place.
...
//Starts with recursion check to avoid chance of segmentation fault during variables declarations.
    ...
    printf("%d, c:%d p:%d ", i, heap[child-1], heap[parent-1]); //DEBUG
    ...
    if (!(child == end && child % 2 == 0))
    {
        ...
        printf("s:%d ", heap[sibling-1]); //DEBUG
